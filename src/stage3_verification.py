        ...
        for s in sentences:
            if not s.cited_indicators:
                if no_citable_evidence:
                    results.append(VerificationResult(
                        ...
                        detail='No HIGH/AMBIGUOUS evidence is available; uncited no-signal statement accepted.'
                    ))
                else:
                    results.append(VerificationResult(
                        sentence_text=s.text, cited_indicators=[], status='FAIL_NO_CITATION',
                        detail='Missing citation tag; violates Stage 2 rule 2.'
                    ))
                continue
            ...
            if invalid_refs:
                results.append(VerificationResult(
                    ...
                    detail=f'Citation references indicators absent from Stage 1: {invalid_refs}.'
                ))
                continue
            ...
            if hedge_check_needed and check_hedge_violation(s.text, lang=lang):
                results.append(VerificationResult(
                    ...
                    detail='Definitive wording used with AMBIGUOUS evidence; violates rule 3.'
                ))
                continue
    ...
    def verify_narrative_uncited(self, sentences: list, facts: list, lang: str = 'ko') -> list:
        ...
        for s in sentences:
            if no_citable_evidence:
                results.append(VerificationResult(
                    sentence_text=s.text, cited_indicators=[], status='PASS_NO_SIGNAL',
                    detail='No HIGH/AMBIGUOUS/RAW evidence is available; no-signal statement accepted.'
                ))
                continue
            ...
            if best_fact.status == 'AMBIGUOUS' and check_hedge_violation(s.text, lang=lang):
                ...
                results.append(VerificationResult(
                    ...
                    detail=(f'entailment={best_scores["entailment"]:.3f} < threshold={self.entailment_threshold} '
                            f'(best-matching fact [{best_fact.indicator}] does not meet the threshold)')
                ))
        return results
...
def summarize_results(results: list) -> dict:
    ...
    #   - conditional_faithfulness_rate / conditional_hallucination_rate
